...
def take_book(body: LoanCreate, db: Session = Depends(get_db),
    ...
    # Notifica interna di conferma prestito (sostituisce l'email)
...
def update_student(student_id: str, body: StudentUpdate, db: Session = Depends(get_db)):
    """Modifica note, stato attivo/inattivo o classe (lato admin)."""
...
def check_overdue_loans():
    ...
    try:
        ...
        for loan in overdue_loans:
            ...
            # Usiamo ">=" anziché "==" sulla soglia per tolleranza: se lo
            # scheduler dovesse saltare un giorno (es. il server va in sleep
            # su un hosting gratuito), l'alert non va perso per sempre — parte
            # comunque al primo controllo successivo. teacher_alert_stage
            # tiene traccia di quale soglia è già stata notificata, per non
            # ricreare la stessa notifica più volte.
            if student.class_id:
